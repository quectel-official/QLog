...
static void insert_diag_qsr4_db_guid_to_list(qshrink4_file_info *db_file_info, int peripheral)
{
    ...
    if (item)
    {
        ...
        strncpy(diagid_guid[guid_list_entry_count].process_name, item->process_name, 30 - 1);
    }
...
static void process_qsr_db_file_read_delayed_rsp(const uint8_t *buf, size_t size)
{
    ...
    if (read_rsp->status == 0)
    {
        ...
        {
            if (parser_state == DB_PARSER_STATE_READ)
            {
                skip_wakeup = 1;
            };
        }
...
static void insert_diag_id_entry(diag_id_entry_struct *entry)
{
    ...
    strncpy(new_entry->process_name, entry->process_name, sizeof(new_entry->process_name) - 1);
...
void mdm_handle_data_for_command_rsp(const uint8_t *buf, size_t size)
{
    if (diag_verbose && (diag_id_state || parser_state))
    {
        ...
        {
            snprintf(str + ((i / 4) * 9), sizeof(str) - ((i / 4) * 9), "%02x%02x%02x%02x ", buf[i + 0], buf[i + 1], buf[i + 2], buf[i + 3]);
        }
        qlog_dbg("rsp=%zd, %s\n", size, str);
    }
    ...
    if (diag_id_state)
    {
        ...
        if (rsp->cmd_code == DIAG_SUBSYS_CMD_F && rsp->subsys_id == DIAG_SUBSYS_DIAG_SERV && qlog_le16(rsp->subsys_cmd_code) == DIAG_GET_DIAG_ID)
        {
    ...
    {
        if ((buf[0] == DIAG_BAD_CMD_F || buf[0] == DIAG_BAD_PARM_F || buf[0] == DIAG_BAD_LEN_F) && buf[1] == DIAG_SUBSYS_CMD_F && buf[2] == DIAG_SUBSYS_LTE)
        {
            qlog_dbg("error %d, buf[0]=%d\n", __LINE__, buf[0]);
    ...
    else if (parser_state)
    {
        ...
        if (rsp->cmd_code == DIAG_SUBSYS_CMD_VER_2_F && rsp->subsys_id == DIAG_SUBSYS_DIAG_SERV && qlog_le16(rsp->subsys_cmd_code) == DIAGDIAG_QSR4_FILE_OP_MODEM)
        {
...
static void mdm_handle_diag_pkt_func(struct diag_pkt *diag_pkt)
{
    if (g_mdm_req != -1)
    {
        ...
        if ((diag_pkt->buf[0] == g_mdm_req) || ((diag_pkt->buf[0] == DIAG_BAD_PARM_F && diag_pkt->buf[1] == g_mdm_req)) || ((diag_pkt->buf[0] == DIAG_BAD_CMD_F && diag_pkt->buf[1] == g_mdm_req)))
        {
...
static void mdm_parse_data_for_command_rsp(const uint8_t *src_ptr, size_t src_length)
{
    if (diag_verbose)
    {
        ...
        {
            snprintf(str + ((i / 4) * 9), sizeof(str) - ((i / 4) * 9), "%02x%02x%02x%02x ", src_ptr[i + 0], src_ptr[i + 1], src_ptr[i + 2], src_ptr[i + 3]);
        }
        qlog_dbg("< %zd, %s\n", src_length, str);
    }
