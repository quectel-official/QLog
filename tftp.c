...
static int tftp_socket(const char *serv_ip)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuse_addr, sizeof(reuse_addr)))
    {
    }
...
int tftp_write_request(const char *filename, long tsize)
{
    ...
    if (tsize)
        size = snprintf(tx_pkt.filename, 512, "%.256s%c%s%c%s%c%d%c%s%c%ld%c", filename, 0, "octet", 0, "blksize", 0, block_size, 0, "tsize", 0, tsize, 0);
    else
        size = snprintf(tx_pkt.filename, 512, "%.256s%c%s%c%s%c%d%c", filename, 0, "octet", 0, "blksize", 0, block_size, 0);
    ...
    for (wait_ack = 0; wait_ack < TFTP_MAX_RETRY; wait_ack++)
    {
        ...
        if (ret >= 4 && tx_pkt.cmd == htons(TFTP_OACK))
        {
            ...
            if(q_req == NULL)
            {
                errno_dbg("failed to create memory for tftp request.");
                return -1;
            }
...
static int tftp_send_data_pkt(int sock, struct tftp_data_packet *tx_pkt, uint16_t block, int size, struct sockaddr_in *to, int is_sync)
{
    ...
    for (wait_ack = 0; wait_ack < TFTP_MAX_RETRY; wait_ack++)
    {
        ...
        {
            dbg("Error Code, Code: %d, block=%u\n", ntohs(rx_pkt.code), block);
            assert(0);
        }
        else
        {
