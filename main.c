...
static int qlog_logfile_create(const char *logfile_dir, const char *logfile_suffix, unsigned logfile_seq)
{
    ...
    if (s_logfile_num && s_logfile_List[logfile_seq % s_logfile_num][0])
    {
        ...
        if (-1 == remove(filename) && errno != EEXIST)
        {
            qlog_dbg("Fail to remove %s! errno : %d (%s)\n", filename, errno, strerror(errno));
        }
    }
...
static int prepare(struct arguments *args)
{
    ...
    use_qmdl2_v2 = 0;
    if (usb_dev->idProduct == 0x0800 || usb_dev->idProduct == 0x0455 || usb_dev->idProduct == 0x0801)
    {
        use_qmdl2_v2 = 1;
    }
    ...
    if (args->ql_dev->idVendor == 0x2c7c && (args->ql_dev->idProduct & 0xF000) == 0x0000 && args->ql_dev->bNumInterfaces == 1 && !args->ql_dev->hardware)
    {
    ...
    if (usb_dev->ttyTHIRD[0] && !force_use_usbfs)
    { // udx710 NMEA
    ...
    else if (usb_dev->third_intf.bInterfaceNumber != 0xFF)
    { // DPL
        static usbfs_read_cfg_t cfg;
...
__restart:
    ...
    if (strStartsWith(args->ttyDM, "/dev/mhi"))
    {
        ...
        strncpy(mhi_dev->ttyDM, args->ttyDM, sizeof(mhi_dev->ttyDM) - 1);
    ...
    else if (strStartsWith(args->ttyDM, "/dev/sdiag"))
    {
        ...
        strncpy(unisoc_dev->ttyDM, args->ttyDM, sizeof(unisoc_dev->ttyDM) - 1);
        strncpy(unisoc_dev->ttyGENERAL, "/dev/slog_nr", sizeof(unisoc_dev->ttyGENERAL) - 1);
    ...
    else if (strStartsWith(args->ttyDM, "/dev/wwan"))
    {
        ...
        strncpy(wwan_dev->ttyDM, args->ttyDM, sizeof(wwan_dev->ttyDM) - 1);
    ...
    if (strStartsWith(args->ttyDM, "/dev/ttyUSB") || strStartsWith(args->ttyDM, "/dev/ttyACM") || strStartsWith(args->ttyDM, "/sys/bus/usb/"))
    {
    ...
    loop_times++;
    if (!is_tftp() && !is_ftp() && !is_tty2tcp() && !is_tcp_client())
    {
        if (-1 == mkdir(args->logdir, 0755) && errno != EEXIST)
        {
            return -1;
        }
    }
		...
		)
    {
        ...
        else
        {
            ...
            if (-1 == mkdir(dump_dir, 0755) && errno != EEXIST)
            {
                qlog_exit_requested = 1;
                goto error;
            }
